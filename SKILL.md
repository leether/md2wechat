...
| 6. 推送命令生成 | ✅（dry-run 时） | 输出可直接复制执行的 ssh/scp 命令 |
| 7. 结构化日志 | ✅ | 写入 `.md2wechat-pipeline.jsonl` |
| 8. 自动 self_report | ✅ | 分析日志，自动捕获新摩擦点，更新 LESSONS_LEARNED.md |
...
# 确认无问题后，去掉 --dry-run 执行推送
${NODE_PATH} ${PIPELINE_HOME}/scripts/orchestrator.mjs \
  --input article.md --account XINZHE --auto-fix

# orchestrator 会输出推送命令，复制执行即可
# （未来版本将直接集成 relay 自动推送）
```
