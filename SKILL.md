...
微信合规 HTML（含 CTA + 群二维码）
  ↓ Step 1.5：本地 Preflight（harness/preflight.mjs）
L1 全绿
...
## Step 1.5：本地 Preflight（推送前拦截，避免返工）

**新增步骤。** 在渲染 HTML 后、准备图片前，先跑本地 preflight，把能在本地拦截的问题全部拦截掉。避免「渲染完 → 压缩完 → 上传到 relay → preflight 报错 → 返工」的浪费。
...
| **本地 preflight** | `${PIPELINE_HOME}/harness/preflight.mjs`（Step 1.5，L1/L2/L3 本地拦截） |
...
30. **bundle 残留旧文件导致推送旧版 HTML**：`/tmp/wechat-*` 目录未清理时，第二次推送可能还是旧文件。使用 `scripts/bundle_wechat_article.mjs` 自动清理并重建 bundle。
31. **digest 长度在 preflight 而非推送时才拦截**：微信 API 限制 digest ≤128 字符，但过去的检查在 relay 上执行，返工成本高。现在 Step 1.5 的本地 preflight 提前拦截。
32. **原文量化描述凭记忆改写导致矛盾**：涉及时间/倍数/人数/版本号等精确数字，必须回源逐字核对。改写时添加的量化描述不等于原文事实。
